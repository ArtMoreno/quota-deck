...
#[command(
    ...
    version,
    about = "QuotaDeck: live AI agent quota in Herdr"
)]
pub struct Cli {
    /// Plugin state directory, equivalent to setting
    /// `$HERDR_PLUGIN_STATE_DIR`.
    ///
    /// Herdr sets that variable for its own hooks, but a statusLine command is
    /// run by the agent CLI rather than by Herdr, so the wrapper has to carry
    /// the directory itself. A `VAR=value command` prefix does that on a POSIX
    /// shell and is a syntax error in `cmd.exe`, which is the shell Claude
    /// Code and Antigravity use on Windows — hence a flag that works on both.
...
pub enum Command {
    ...
    Configure {
        ...
        /// Provider mark set: icon (default), unicode, or off.
        #[arg(long, value_parser = parse_glyph_set)]
        brand_glyphs: Option<GlyphSet>,
        ...
        /// Ask a running Herdr server to re-read its config once the write
        /// succeeds. The plugin's own actions pass this; upstream chained
        /// `herdr server reload-config` in the shell, which Windows has no
        /// equivalent for in a command vector.
        #[arg(long)]
        reload_herdr: bool,
    },
    ...
    OpenSettings,
    /// Open the QuotaDeck dashboard as a popup.
    OpenDashboard,
    /// Open the QuotaDeck dashboard beside the currently focused pane.
    OpenDashboardSplit,
    ...
    AgyStatusline,
    /// Parse Herdr action JSON for the POSIX installer. Internal plumbing.
    #[command(hide = true)]
    ParseHerdrActionJson {
...
impl ProviderSelection {
    pub fn providers(self) -> Vec<Provider> {
        match self {
            ...
            Self::Openrouter => vec![Provider::OpenRouter],
        }
    }
}
...
/// Which quota fields the sidebar shows. Prompt-derived topic is opt-in.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl Default for FieldSet {
    fn default() -> Self {
        Self::all().toggled(SidebarField::Topic)
    }
}
...
fn parse_glyph_set(value: &str) -> Result<GlyphSet, String> {
    GlyphSet::parse(value).ok_or_else(|| "brand-glyphs must be icon, unicode, or off".to_string())
}
...
pub struct ConfigureOptions {
    ...
    pub brand_glyphs: Option<GlyphSet>,
...
impl AgentOrder {
    ...
    pub const LABEL: &'static str = "low first";
...
impl AgentSelection {
    ...
    /// Selection for an uninstall run.
    ///
    /// No CLI selection means a full uninstall. Installer wrappers use the
    /// one-shot preference because Herdr actions have a fixed command line.
    pub fn from_uninstall_args(values: &[Self]) -> Result<Vec<Harness>, String> {
        if !values.is_empty() {
            return Ok(Self::resolve(values));
        }
        let Some(raw) = crate::prefs::read(crate::prefs::UNINSTALL_AGENTS) else {
            return Ok(Self::SUPPORTED.to_vec());
        };
        let mut parsed = Vec::new();
        for name in raw.split(',').map(str::trim) {
            let Some(agent) = Self::parse(name) else {
                return Err(format!("invalid uninstall agent `{name}`"));
            };
            parsed.push(agent);
        }
        if parsed.is_empty() {
            return Err("uninstall agent selection is empty".to_string());
        }
...
mod tests {
    ...
    #[test]
    fn every_supported_agent_round_trips_through_the_string_parser() {
        // clap's ValueEnum accepts `--agent hermes` on its own, but the
        // settings pane and the config directory both go through `parse`.
        // A harness missing here is dropped on every persisted path, which
        // uninstalls it instead of configuring it.
        for harness in AgentSelection::SUPPORTED {
            let name = crate::settings::agent_name(harness);
            let parsed = AgentSelection::parse(name)
                .unwrap_or_else(|| panic!("`{name}` is not accepted by AgentSelection::parse"));
            assert_eq!(parsed.harness(), Some(harness), "{name}");
        }
    }

    #[test]
    fn one_shot_uninstall_selection_rejects_unknown_and_mixed_names() {
        let directory = tempfile::tempdir().unwrap();
        crate::prefs::testing::with_config_dir(directory.path(), || {
            for value in ["grokk", "grok,grokk"] {
                crate::prefs::write(crate::prefs::UNINSTALL_AGENTS, value).unwrap();
                assert!(AgentSelection::from_uninstall_args(&[]).is_err(), "{value}");
            }
        });
    }

    #[test]
    fn configure_accepts_each_brand_glyph_set() {
        for (value, expected) in [
            ("icon", GlyphSet::IconFont),
            ("unicode", GlyphSet::Unicode),
            ("off", GlyphSet::Off),
        ] {
            let cli =
                Cli::try_parse_from(["herdr-agent-quota", "configure", "--brand-glyphs", value])
                    .unwrap();
            let Command::Configure { brand_glyphs, .. } = cli.command else {
                unreachable!()
            };
            assert_eq!(brand_glyphs, Some(expected));
        }
    }
}
