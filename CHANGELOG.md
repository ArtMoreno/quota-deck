...
- Bundle an eight-provider icon-only fallback font and WezTerm configuration
  example to fix missing brand glyphs. Includes OpenRouter, which the original
  Herdr Agent Icons Max font lacks.
- Document font-directory setup, configuration merging, reload, and verification.
  Existing terminal settings are never overwritten automatically.

## [1.4.2] - 2026-09-04

- Windows setup now installs and repairs the `quotadeck` command. It runs the
  dashboard in the current pane and returns to the shell on exit, without
  opening an extra split or printing a Herdr action receipt.
- Preserve unrelated custom commands and remove only the managed launcher
  during full uninstall.
...
- Claude quota and reset windows now refresh directly from Claude Code's local
  OAuth credential, with the existing statusLine observation retained as a
  fallback and for per-session diagnostics.
- The user-facing product name is **QuotaDeck**; the plugin id and binary name
  remain unchanged so existing installations upgrade in place.
- One-click actions open the dashboard either as a sized popup card or as
  a real resizable Herdr split pane.
- The dashboard can refresh OpenCode Go directly from OpenCode's exact stored
  `opencode-go` credential when its opt-in row is enabled, without waiting for
  a Go-backed chat turn.
- Dashboard settings can show, hide, reorder, recolor, and select fields for
  each provider. Plugin-managed sidebar labels reuse those saved colors; Pi
  inherits Codex and OpenCode inherits OpenCode Go.
- Persistent caches use domain-separated hashes for account and session
  identifiers and omit prompt text, session summaries, and full provider
  payloads. Legacy caches are sanitized during migration.
- Prompt-derived topic metadata is opt-in and its local Herdr 24-hour TTL is
  disclosed. Exact statusLine restoration keeps the only full settings backup
  beside the source file and stores just a digest in plugin state.

### Fixed

- Install/uninstall helpers recognize Herdr 0.8.2's nested action log ID and
  wait for the matching completion; missing optional OMP no longer blocks setup.
- Successful pane launches and installer registration no longer print raw JSON.
  Dashboard Settings stays in the current Herdr session, and configuration
  respects native custom-path and XDG overrides.
- Windows pane actions pass a normal plugin working directory to Herdr 0.8.2,
  avoiding its long-path-prefixed ConPTY launch failure.
- Dashboard repainting uses the terminal alternate screen, so resizing no
  longer stores every former responsive layout in pane scrollback.
- Compact panes scroll only the current provider list, with bounded mouse-wheel,
  arrow, PageUp/PageDown, Home, and End controls plus a visible position hint.
- Claude's global dashboard never borrows quota from an arbitrary cached
  session; an empty tick may reuse only the same session's unexpired windows.
- The popup is tall enough to show OpenCode Go's 5h, 7d, and 30d windows plus
  the footer without clipping.
- Dashboard refreshes are queued off the UI thread, coalesce duplicate work,
  and serialize local/database access across dashboard instances.
- Untouched Herdr configuration is restored byte-for-byte on uninstall;
  intervening user edits are preserved while QuotaDeck-owned entries are removed.
- Partial uninstall rejects unknown agent names, action polling fails closed,
  and a temporarily enabled plugin returns to its prior disabled state.

## [1.4.0] - 2026-09-03

First release of the `herdr-agent-quota-win` fork of
[levi-qiao/herdr-agent-quota](https://github.com/levi-qiao/herdr-agent-quota).
The plugin id changes to `herdr-agent-quota-win` so the two never collide in one
Herdr install; the binary name is unchanged.

### Added

- **Windows is a supported platform.** `platforms` now includes `windows`, and
  a new `src/platform.rs` holds every difference: the Herdr control socket is a
  named pipe rather than `AF_UNIX`, `%USERPROFILE%` backs up an unset `$HOME`,
  a chained statusLine runs through `cmd.exe /D /C`, process groups use
  `CREATE_NEW_PROCESS_GROUP` and `taskkill /T`, and the watcher detaches with
  `DETACHED_PROCESS`.
- The settings pane and both installer scripts can persist the `icon`,
  `unicode`, or `off` brand-glyph set.
- Windows-native process fixtures, bounded integration-test subprocesses, and a
  real named-pipe round-trip test cover clean Windows installations.
- **Hermes collector.** Nous Portal dollars, read from the same
  `/api/oauth/account` endpoint Hermes' own `/usage` surface uses, with the
  token Hermes already stored. Plan dollars and purchased top-up dollars are
  separate windows because plan renews and top-up rolls over.
- **OpenRouter collector.** Credit balance from `/api/v1/credits`, preferring
  the per-key spend limit from `/auth/key` when the key carries one. It has no
  harness, so it is refreshed globally and shown in the dashboard rather than
  owning a sidebar row. Hermes plan dollars carry their real renewal time;
  top-up and OpenRouter balances do not get an invented reset. When no explicit
  plugin key source exists, it reuses `OPENROUTER_API_KEY` from the active
  Hermes home's `.env` without copying or persisting the secret.
- **OpenCode local usage.** The dashboard now always shows OpenCode's real
  30-day token total and recorded spend from its read-only local database.
  Dollars are labeled as spent, not remaining quota; OpenCode Go stays a
  separate conditional quota row.
- **Brand marks.** A `brand-glyphs` setting drawing each provider's logo beside
  its name — `icon` (the `Herdr Agent Icons Max` private-use codepoints),
  `unicode` (marks any monospace font carries), or `off`. Real SVG marks and
  matching transparent 64px PNGs ship in `docs/icons/`.
- `install.ps1` and `uninstall.ps1`, the Windows counterparts of the shell
  installers, including `-OpenRouterKey` to hand the collector a key without
  exporting it into the Herdr server's environment.
- A global `--state-dir` flag. A leading `VAR=value` assignment is POSIX shell
  syntax with no `cmd.exe` equivalent, so the statusLine wrapper carries the
  plugin state directory as an argument instead.
- `configure --reload-herdr` and `open-settings` subcommands, replacing two
  manifest entries that previously chained commands with `sh -c`. Herdr runs
  argv arrays with no shell, so the chaining had to move into the binary.
- `THIRD-PARTY-NOTICES.md`, and a Windows job in CI.

### Changed

- Packed sidebar rows now match the two-line design: context/cache/TTL
  together, then both quota windows. Stacked mode keeps one field per row.
- Dashboard and settings popups now use opaque cards, compact
  labels, semantic colors, full-row selection, and bounded no-scroll rendering.
- Manifest commands are bare argv vectors rather than `sh -c` strings, matching
  what Herdr documents and actually does.
- Herdr's config directory resolves to `%APPDATA%\herdr` on Windows. Writing to
  `~/.config/herdr` there produced a file the server never read.
- OpenCode's store falls back to `%LOCALAPPDATA%` when XDG variables are unset.
- The minimum supported Herdr version is now 0.8.2, the version against which
  the plugin's explicit popup sizing is verified.

### Fixed

- Hermes top-up quota is emitted only with a real positive Portal denominator;
  missing or inconsistent totals can no longer fabricate a healthy percentage.
- Hermes plan and top-up use the sidebar's two existing quota slots, so neither
  can hide the other and no new metadata schema is required.
...
- Local pane identity no longer strips the billing provider's brand mark.
- Herdr keybindings address the fork's `herdr-agent-quota-win` action ids.
- The plugin-manifest CI validator emits valid Python when reporting errors.
- Claude settings are restored byte-for-byte after an untouched install, while
  later user edits survive uninstall.
- Windows named-pipe requests now honor their deadline when a server accepts a
  connection but never replies.
- Full uninstall ignores a persisted subset; action-driven partial uninstall
  uses a consumed one-shot selection and leaves the saved install scope intact.
- The Windows build now creates the extensionless runtime path used by manifest
  commands; Herdr 0.8.2 does not append `.exe` when launching that path.
...
- Claude/Agy quota windows are remembered per statusLine session so a work
  ...
  borrowing another account's numbers. This follows the upstream fix in #30.
