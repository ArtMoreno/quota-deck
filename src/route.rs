...
pub fn resolve_with_identity(pane: &AgentPane) -> ResolvedPane {
    let resolution = match pane.harness {
        Harness::Codex | Harness::Grok | Harness::Claude | Harness::Agy | Harness::Hermes => pane
            .harness
            .billing()
...
mod tests {
    ...
    /// Synthetic omp v18-shaped data covers the padded `title` header, slash
    /// model selector, provider switch, and credential pins without retaining
    /// a real transcript, session id, prompt, or account-derived hash.
    #[test]
