...
const PRIVATE_ID_PREFIX: &str = "sha256:";

pub(crate) fn private_identifier(domain: &str, value: &str) -> String {
    let prefix = format!("{PRIVATE_ID_PREFIX}{domain}:");
...
pub(crate) fn persisted_private_identifier(domain: &str, value: &str) -> String {
    if let Some(digest) = value
        .strip_prefix(PRIVATE_ID_PREFIX)
        .filter(|digest| digest.len() == 64 && digest.bytes().all(|byte| byte.is_ascii_hexdigit()))
    {
        return format!(
            "{PRIVATE_ID_PREFIX}{domain}:{}",
            digest.to_ascii_lowercase()
        );
    }
    private_identifier(domain, value)
}
...
pub enum Provider {
    ...
    Omp,
    /// Hermes' Nous Portal account. Dollar-denominated rather than a renewing
    /// percentage window; see `providers::hermes`.
    Hermes,
    /// An OpenRouter credit balance. Unlike every other collector this has no
    /// harness of its own: it is an account that other harnesses spend from,
    /// so it is refreshed globally and never resolved from a pane.
    OpenRouter,
}
...
impl Provider {
    ...
    pub fn source(self) -> &'static str {
        match self {
            ...
            Self::Omp => "omp-usage",
            Self::Hermes => "hermes-portal",
            Self::OpenRouter => "openrouter-credits",
        }
    }
}
...
impl Harness {
    ...
    pub fn billing(self) -> Option<Billing> {
        match self {
            ...
            Self::Hermes => Some(Provider::Hermes),
...
impl CacheUsage {
    ...
    pub fn with_session_totals(
        ...
        self.session_id = Some(private_identifier("session", &session_id));
        self.transcript_offset = transcript_offset;
        self
    }
}

fn protect_context_session(context: Option<&mut ContextUsage>) {
    if let Some(cache) = context.and_then(|context| context.cache.as_mut()) {
        cache.session_id = cache
            .session_id
            .take()
            .map(|value| persisted_private_identifier("session", &value));
    }
}

fn protect_session_map<T>(map: &mut BTreeMap<String, T>) {
    *map = std::mem::take(map)
        .into_iter()
        .map(|(key, value)| (persisted_private_identifier("session", &key), value))
        .collect();
}
...
impl ProviderSnapshot {
    ...
    pub fn model_for_session(&self, session_id: Option<&str>) -> Option<&str> {
        ...
        self.session_models
            .get(&private_identifier("session", session_id))
            .map(String::as_str)
    }
    ...
    pub fn with_account_id(mut self, account_id: Option<String>) -> Self {
        self.account_id = account_id.map(|value| private_identifier("account", &value));
        self
    }

    pub(crate) fn protect_private_fields(&mut self) {
        self.session_summaries.clear();
        self.account_id = self
            .account_id
            .take()
            .map(|value| persisted_private_identifier("account", &value));
        protect_session_map(&mut self.session_models);
        protect_session_map(&mut self.session_contexts);
        protect_session_map(&mut self.session_windows);
        protect_context_session(self.context.as_mut());
        for context in self.session_contexts.values_mut() {
            protect_context_session(Some(context));
        }
    }
    ...
    pub fn usable_for_account(
        ...
        match (self.account_id.as_deref(), current_account_id) {
            (Some(saved), Some(current)) => {
                private_identifier("account", saved) == private_identifier("account", current)
            }
            (Some(_), None) => false,
    ...
    pub fn severity_for_windows(
        ...
        let relevant = match provider {
            // Dollar balances have no short window; scoring them on a missing
            // 5h would report Unknown for an account that is simply not
            // period-based.
            Provider::Grok | Provider::Hermes | Provider::OpenRouter => long_window(windows),
            Provider::Codex
            | Provider::Claude
            | Provider::Agy
            | Provider::OpenCodeGo
...
mod tests {
    ...
    #[test]
    fn private_identifiers_are_domain_tagged_and_raw_digest_shapes_are_rehashed() {
        let raw_digest = format!("sha256:{}", "a".repeat(64));
        let session = private_identifier("session", &raw_digest);
        let account = private_identifier("account", &raw_digest);
        assert_ne!(session, raw_digest);
        assert_ne!(session, account);
        assert_eq!(private_identifier("session", &session), session);
        assert_ne!(private_identifier("account", &session), session);
        assert_eq!(
            persisted_private_identifier("session", &raw_digest),
            format!("sha256:session:{}", "a".repeat(64))
        );
    }
