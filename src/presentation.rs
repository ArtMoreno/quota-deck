...
use crate::dashboard_prefs::{DashboardField, ProviderPreference};
use crate::model::{
...
pub struct MetadataTokens {
    ...
    /// One compact token per window (`5h 42% reset 4h07m`), severity chooses the hue.
    pub quota_5h: String,
...
/// How a sidebar row is rendered: the number the percentages state, and the
/// brand marks drawn beside provider names.
///
/// One value rather than two parallel parameters — every function that carries
/// one carries the other, and splitting them invites a call site that passes a
/// stale half.
...
///
/// Every renderer that predates brand marks passes a `PercentStyle`; this keeps
/// those call sites reading the way they did instead of restating the default.
impl From<PercentStyle> for RowStyle {
    fn from(percent: PercentStyle) -> Self {
        Self {
            percent,
...
impl MetadataTokens {
    pub fn from_snapshot(snapshot: &ProviderSnapshot, now_unix: u64) -> Self {
        Self::from_snapshot_for_session(snapshot, now_unix, None, RowStyle::default())
    }
    ...
    fn from_snapshot_parts(
        ...
        let style = style.into();
        // The brand mark rides on the provider token so it inherits that
        // token's own colour from Herdr's theme and wraps with it.
        let quota_provider = style
            .glyphs
            .label(snapshot.provider, snapshot.provider.display_name());
        ...
        Self {
            ...
            quota_provider,
            quota_model,
            quota_5h_severity: first_window
            ...
            quota_error: None,
            quota_headroom: headroom(snapshot.provider, windows),
        }
    }
    ...
    pub fn unavailable(provider: Provider, reason: impl Into<String>) -> Self {
        Self::unavailable_with(provider, reason, GlyphSet::default())
    }

    /// [`Self::unavailable`] with an explicit mark set, for the renderers that
    /// have the user's configured one in hand.
    pub fn unavailable_with(
        provider: Provider,
        reason: impl Into<String>,
        glyphs: GlyphSet,
    ) -> Self {
        let quota_provider = glyphs.label(provider, provider.display_name());
        Self {
...
/// The least remaining quota across the two slots the sidebar shows.
///
/// Deliberately resolved through [`sidebar_windows`] so a sort or alert is
/// always explained by a visible value. Hermes truthfully uses its monthly
/// plan in the first slot and its non-renewing top-up in the second.
///
/// Rounded down, so a window one point above a threshold is never rounded
/// onto the wrong side of it.
fn headroom(provider: Provider, windows: &[UsageWindow]) -> Option<u8> {
    let (first, second) = sidebar_windows(provider, windows);
    first
        .into_iter()
        .chain(second)
...
/// The dashboard has room for every window. The sidebar has two reusable
/// slots: most providers show 5h plus their long window, while Hermes shows
/// its monthly plan plus weekly top-up. A 30d value is never relabeled as 7d.
pub fn dashboard_summary(
    snapshot: &ProviderSnapshot,
    now_unix: u64,
    style: PercentStyle,
) -> String {
    dashboard_segments(snapshot, now_unix, style)
        .into_iter()
        .map(|(value, _)| value)
        .collect::<Vec<_>>()
        .join(" · ")
...
pub(crate) fn dashboard_segments(
    snapshot: &ProviderSnapshot,
    now_unix: u64,
    style: PercentStyle,
) -> Vec<(String, Severity)> {
    ...
    kinds
        ...
        .map(|window| {
            (
                format_window(window, now_unix, false, style),
...
fn filtered_window(
    ...
    let (amount_field, percent_field, reset_field) = match (provider, window.kind) {
        ...
        (Provider::Hermes, _) => (Some(TopUpAmount), TopUpPercent, None),
        (Provider::OpenRouter, _) => (Some(CreditsAmount), CreditsPercent, None),
        (_, WindowKind::FiveHour) => (None, ShortPercent, Some(ShortReset)),
        _ => (None, LongPercent, Some(LongReset)),
    };

    let label = dashboard_label(window.display_label());
    let (base, amount) = label
        .rsplit_once('$')
        .map(|(base, amount)| (base.trim_end(), Some("$".to_string() + amount)))
        .unwrap_or((label.as_str(), None));
    let show_amount = amount_field.is_some_and(|field| preference.has(field)) && amount.is_some();
    let show_percent = preference.has(percent_field);
    let show_reset =
        reset_field.is_some_and(|field| preference.has(field)) && window.resets_at.is_some();
...
fn missing_five_hour_label(provider: Provider) -> Option<&'static str> {
    // Claude/Agy keep a visible placeholder in the first quota slot. Other
    // providers omit a period they do not report; Hermes maps its plan there.
    match provider {
        ...
        | Provider::OpenCodeGo
        | Provider::Omp
        // Neither balance has a 5h period, so a placeholder would imply one.
        | Provider::Hermes
...
fn format_window(
    ...
    let label = format!(
        "{} {percent}{suffix}",
        dashboard_label(window.display_label())
    );
...
fn dashboard_label(label: &str) -> String {
    let Some((prefix, amount)) = label.rsplit_once('$') else {
        return label.to_string();
    };
    let Ok(amount) = amount.parse::<f64>() else {
        return label.to_string();
    };
    format!("{prefix}${amount:.2}")
}
...
mod tests {
    ...
    /// explainable by a token the user can see, so an unselected monthly
    /// window must not decide either.
    #[test]
    ...
    fn a_monthly_only_plan_fills_the_long_window_slot_with_its_own_label() {
        ...
        assert_eq!(values.quota_week, "30d 70% reset 17d8h");
    ...
    fn a_weekly_window_always_wins_the_long_window_slot() {
        ...
        let values = MetadataTokens::from_snapshot(&snapshot, 0);
        assert_eq!(values.quota_week, "7d 80% reset 2d3h");
        assert!(!values.quota_week.contains("30d"));
    }

    #[test]
    fn hermes_uses_both_existing_slots_for_plan_and_top_up() {
        let snapshot = ProviderSnapshot::new(
            Provider::Hermes,
            vec![
                window(WindowKind::Monthly, 75.0, 1_500_000)
                    .with_source_window("plan $15.00", None),
                UsageWindow::new(WindowKind::Weekly, 35.0, None)
                    .unwrap()
                    .with_source_window("top-up $8.00", None),
            ],
            0,
        );
        let values = MetadataTokens::from_snapshot(&snapshot, 0);
        assert_eq!(values.quota_5h, "plan $15.00 25% reset 17d8h");
    ...
    #[test]
    fn hermes_keeps_plan_before_topup_with_dashboard_money_precision() {
        let snapshot = ProviderSnapshot::new(
            Provider::Hermes,
            vec![
                window(WindowKind::Monthly, 75.0, 27 * 24 * 60 * 60)
                    .with_source_window("plan $15.0", None),
                UsageWindow::new(WindowKind::Weekly, 35.0, None)
                    .unwrap()
                    .with_source_window("top-up $8.00", None),
            ],
        ...
        assert_eq!(
            dashboard_summary(&snapshot, 0, PercentStyle::default()),
            "plan $15.00 25% reset 27d · top-up $8.00 65%"
        );
    }
    ...
    fn the_used_style_flips_the_sidebar_number_but_not_its_width_or_colour() {
        ...
        assert_eq!(remaining.quota_5h, "5h 42% reset 4h07m");
        assert_eq!(remaining.quota_week, "7d 73% reset 2d3h");

        let used =
            MetadataTokens::from_snapshot_for_session(&snapshot, 0, None, PercentStyle::Used);
        assert_eq!(used.quota_5h, "5h 58% reset 4h07m");
        assert_eq!(used.quota_week, "7d 27% reset 2d3h");
    ...
    fn metadata_uses_compact_quota_window_labels() {
        ...
        let values = MetadataTokens::from_snapshot(&snapshot, 0);
        assert_eq!(values.quota_5h, "5h 42% reset 4h07m");
        assert_eq!(values.quota_week, "7d 73% reset 2d3h");
    ...
    fn metadata_uses_the_model_for_the_pane_session() {
        ...
        // The default mark set prefixes the provider half of the token.
        assert_eq!(
            session_one.quota_provider_model,
            format!(
                "{}/Sonnet",
                GlyphSet::default().label(Provider::Claude, "Claude")
            )
        );
        ...
        assert_eq!(
            session_two.quota_provider_model,
            GlyphSet::default().label(Provider::Claude, "Claude")
        );
    }
    ...
    fn pane_without_session_id_does_not_broadcast_local_diagnostics() {
        ...
        assert_eq!(
            values.quota_provider_model,
            format!(
                "{}/grok-4.6",
                GlyphSet::default().label(Provider::Grok, "Grok")
            )
        );
    ...
    fn weekly_only_sidebar_window_uses_the_compact_reset_eta() {
        ...
        let values = MetadataTokens::from_snapshot(&snapshot, 0);
        assert_eq!(values.quota_week, "7d 69% reset 6d");
        assert_eq!(values.quota_5h, "");
    ...
    fn grok_and_codex_panes_keep_account_quota_when_the_session_is_known() {
        ...
        assert_eq!(grok_pane.quota_week, "7d 69% reset 6d");
        ...
        assert_eq!(codex_pane.quota_5h, "5h 60% reset 4h07m");
        assert_eq!(codex_pane.quota_week, "7d 69% reset 6d");
    }
    ...
    fn claude_pane_quota_follows_the_pane_session() {
        ...
        assert_eq!(work.quota_5h, "5h 82% reset 4h07m");
        assert_eq!(work.quota_week, "7d 90% reset 6d");
        ...
        assert_eq!(personal.quota_5h, "5h 18% reset 4h07m");
        assert_eq!(personal.quota_week, "7d 10% reset 6d");
