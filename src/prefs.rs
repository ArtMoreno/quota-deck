...
/// One-shot partial-uninstall selection consumed before configuration runs.
pub const UNINSTALL_AGENTS: &str = "uninstall-agents";
...
pub const BRAND_GLYPHS: &str = "brand-glyphs";
...
mod tests {
    ...
    #[test]
    fn full_cleanup_includes_the_installer_owned_openrouter_key() {
        let directory = tempfile::tempdir().unwrap();
        testing::with_config_dir(directory.path(), || {
            for name in ALL {
                write(name, "value").unwrap();
            }
            for name in ALL {
                clear(name).unwrap();
