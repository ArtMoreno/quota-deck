...
pub fn ensure_omp(agents: &[Harness]) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        if detail.contains("omp extension directory not found") {
            println!("OMP is not installed; skipped its optional integration. After installing OMP, run Install / repair QuotaDeck again.");
            return Ok(());
        }
