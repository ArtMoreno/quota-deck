...
This Windows fork is authored and maintained by
[Art Moreno](https://github.com/ArtMoreno).

Commit authors and `Co-authored-by` trailers identify human contributors only.
Do not add Claude, ChatGPT, Codex, or other assistant accounts as authors or
co-authors. Preserve the upstream Levi Qiao MIT notice and third-party licenses.
...
```sh
git clone https://github.com/ArtMoreno/quota-deck
cd quota-deck
./install.sh
```
...
2. **Do not mutate provider credentials.** Runtime collectors never refresh,
   rotate, cache, or log a token. An installer may persist only a credential the
   user explicitly passed, with clear disclosure, per-user protection, and
   reliable full-uninstall cleanup.
