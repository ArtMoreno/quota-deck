...
/// Extract the invoked log id or one matching log's status from Herdr JSON.
/// The shell installers call this hidden command instead of scraping JSON.
pub fn parse_action_json(input: &str, log_id: Option<&str>) -> Result<String> {
    ...
    if let Some(log_id) = log_id {
        ...
        return log
            ...
            .map(str::to_string)
            .with_context(|| format!("Herdr log {log_id} has no status"));
    }
    value
...
const OBSOLETE_METADATA_TOKEN_NAMES: [&str; 19] = [
    "quota_icon",
    "quota_state",
    "quota_status",
    "quota_summary",
...
/// even when the budget is tight: the identity and live diagnostics.
const ROWS_THAT_MUST_NOT_LAG: [&str; 8] = [
    "quota_provider",
    "quota_model",
    "quota_provider_model",
    "quota_topic",
...
/// This plugin's Herdr id, as declared in `herdr-plugin.toml`.
///
/// It differs from upstream's on purpose: two plugins cannot share an id in one
/// Herdr install, and the marketplace index would otherwise carry two entries
/// nobody could tell apart. Anything addressing the plugin *through Herdr* must
/// use this. The markers written into a user's own config deliberately keep
/// upstream's name, so migrating from upstream cleanly reclaims its rows rather
/// than leaving them orphaned.
pub const PLUGIN_ID: &str = "herdr-agent-quota-win";

// Herdr rejects an agent-view source that is not `plugin:<manifest id>`
// with `plugin_not_found`, so this must track PLUGIN_ID and not the
// markers written into the user's own config, which deliberately keep
// upstream's name.
const AGENT_VIEW_SOURCE: &str = "plugin:herdr-agent-quota-win";
...
fn socket_request(payload: &Value) -> Result<Option<Value>> {
    ...
    // Unix domain socket on macOS/Linux, named pipe on Windows. Both are one
    // newline-delimited request and one reply on a connection nobody keeps.
    let mut stream = crate::platform::HerdrSocket::connect(&path, SOCKET_TIMEOUT)?;
    writeln!(stream, "{payload}").context("send Herdr socket request")?;
...
/// Ask a running Herdr server to re-read its configuration.
///
/// The upstream manifest chained `&& herdr server reload-config` onto the
/// configure action inside `sh -c`. There is no shell in the command vector on
/// Windows, so the step lives here instead — which also means a failure to
/// reload is reported rather than swallowed by the shell's exit status.
///
/// A missing server is not an error: `configure` is legitimately run from a
/// plain terminal with no Herdr running, and the config is read at startup.
pub fn reload_config() -> Result<()> {
    ...
    match output {
        ...
        Ok(output) => {
            eprintln!(
                "warning: herdr server reload-config failed: {}",
                String::from_utf8_lossy(&output.stderr).trim()
            );
            Ok(())
        }
        Err(error) => {
            eprintln!("warning: could not run herdr server reload-config: {error}");
            Ok(())
        }
    }
}

/// Open this plugin's settings popup.
///
/// Also previously a `sh -c` one-liner in the manifest. Spawning it directly
/// keeps the action working on Windows and lets the pane id stay in one place.
pub fn open_settings_pane() -> Result<()> {
    open_plugin_pane(if cfg!(windows) {
        "settings-windows"
    } else {
        "settings"
    })
...
/// Invoke the registered action so Herdr supplies the focused-pane context
/// needed when the dashboard itself asks to open Settings.
pub fn invoke_settings_action() -> Result<()> {
    let executable = std::env::var_os("HERDR_BIN_PATH").unwrap_or_else(|| "herdr".into());
    let output = Command::new(executable)
        .args([
            "plugin",
        ...
        .output()
        .context("invoke the QuotaDeck settings action")?;
    if !output.status.success() {
        anyhow::bail!(
            "Herdr settings action failed: {}",
            String::from_utf8_lossy(&output.stderr).trim()
        );
    }
    Ok(())
...
pub fn open_dashboard_split() -> Result<()> {
    open_plugin_pane(if cfg!(windows) {
        "dashboard-split-windows"
    } else {
        "dashboard-split"
    })
}

fn open_plugin_pane(entrypoint: &str) -> Result<()> {
    let executable = std::env::var_os("HERDR_BIN_PATH").unwrap_or_else(|| "herdr".into());
    let cwd = herdr_cli_path(&std::env::current_dir().context("read plugin working directory")?);
    let output = Command::new(&executable)
        .args([
            "plugin",
            "pane",
        ...
        .args(["--focus"])
        .output()
        .with_context(|| format!("open the {entrypoint} plugin pane"))?;
    if !output.status.success() {
        anyhow::bail!(
...
fn herdr_cli_path(path: &Path) -> PathBuf {
...
fn apply_identity(tokens: &mut BTreeMap<String, String>, identity: &PaneIdentity) {
    let provider = tokens
        ...
        .map(|(mark, _)| format!("{mark} {}", identity.provider))
    ...
    if identity.model.is_empty() {
        ...
        tokens.insert(
            "quota_provider_model".to_string(),
            format!("{provider}/{}", identity.model),
        );
    }
}
...
mod tests {
    ...
    #[test]
    fn action_json_parser_requires_structural_ids_and_statuses() {
        assert_eq!(
            parse_action_json(
                r#"{"result":{"log":{"log_id":"plugin-log-8","status":"running"}}}"#,
                None
            )
            .unwrap(),
            "plugin-log-8"
        );
        let invoked = r#"{"result":{"log_id":"plugin-log-7"}}"#;
        assert_eq!(parse_action_json(invoked, None).unwrap(), "plugin-log-7");
        let listed = r#"{"result":{"logs":[{"log_id":"plugin-log-7","status":"succeeded"}]}}"#;
        assert_eq!(
            parse_action_json(listed, Some("plugin-log-7")).unwrap(),
            "succeeded"
        );
        assert_eq!(
            parse_action_json(listed, Some("plugin-log-pending")).unwrap(),
            ""
        );
    ...
    #[test]
    fn local_identity_keeps_the_quota_provider_brand_mark() {
        let marked = crate::brand::GlyphSet::Unicode.label(Provider::Claude, "Claude");
        let mut tokens = BTreeMap::from([
            ("quota_provider".to_string(), marked.clone()),
            ("quota_provider_model".to_string(), marked),
        ]);
        ...
        assert_eq!(tokens.get("quota_provider").unwrap(), "✳ Claude");
        assert_eq!(
            tokens.get("quota_provider_model").unwrap(),
            "✳ Claude/Sonnet"
        );
    }
    ...
    #[test]
    fn a_pane_with_every_obsolete_name_is_cleaned_in_bounded_reports() {
        let tokens = OBSOLETE_METADATA_TOKEN_NAMES
            .into_iter()
            .chain(LEGACY_METADATA_TOKEN_NAMES)
        ...
        assert!(!metadata_matches(&pane.tokens, &BTreeMap::new()));
        assert!(metadata_report_names(&pane, &BTreeMap::new()).len() <= MAX_METADATA_TOKENS);
    }

    #[test]
    fn weekly_only_uses_the_dedicated_week_slot_inside_the_metadata_cap() {
    ...
    #[test]
    fn claude_placeholder_and_week_use_the_two_dedicated_slots() {
        let snapshot = crate::model::ProviderSnapshot::new(
            Provider::Claude,
            vec![crate::model::UsageWindow::new(
                crate::model::WindowKind::Weekly,
    ...
    #[test]
    fn empty_five_hour_still_publishes_the_dedicated_week_slot() {
        let snapshot = crate::model::ProviderSnapshot::new(
            Provider::Codex,
            vec![crate::model::UsageWindow::new(
                crate::model::WindowKind::Weekly,
    ...
    #[test]
    fn present_five_hour_and_week_use_the_two_dedicated_tokens() {
        let snapshot = crate::model::ProviderSnapshot::new(
            Provider::Codex,
            vec![
    ...
    #[test]
    fn stale_inline_week_is_cleared_while_the_dedicated_week_is_published() {
        let snapshot = crate::model::ProviderSnapshot::new(
            Provider::Grok,
            vec![crate::model::UsageWindow::new(
                crate::model::WindowKind::Weekly,
