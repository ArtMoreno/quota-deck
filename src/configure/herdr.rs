...
const REFRESH_ACTION: &str = "herdr-agent-quota-win.refresh";
const SETTINGS_KEY: &str = "prefix+shift+q";
const SETTINGS_ACTION: &str = "herdr-agent-quota-win.open-settings";
const DASHBOARD_KEY: &str = "prefix+shift+d";
const DASHBOARD_ACTION: &str = "herdr-agent-quota-win.open-dashboard-split";
const CONFIG_PRESENCE_FILE: &str = "herdr-config.original.present";
const CONFIG_INSTALLED_FILE: &str = "herdr-config.installed.toml";
...
const PROVIDER_STYLES: [(Harness, &str, Option<&str>, Option<&str>); 8] = [
    ...
    (Harness::Hermes, "hermes", Some("#cfd6e4"), Some("#acb0b7")),
];
...
pub fn check(
    ...
    let updated =
        add_quota_row_with_dashboard(&original, agents, layout, row_gap, fields, brand, dashboard)?;
    if updated == original {
        println!(
...
pub fn apply(
    ...
    let exact_restore = exact_restore_is_current(&original)?;
    ...
    record_installed_config(&updated, exact_restore)?;
...
pub fn uninstall(agents: &[Harness], full: bool) -> Result<()> {
    ...
    if let Some(installed) = installed_path()? {
        remove_if_exists(&installed).context("remove Herdr installed-config snapshot")?;
    }
    Ok(())
}
...
pub fn config_path() -> Result<PathBuf> {
    if let Some(path) =
        std::env::var_os("HERDR_CONFIG_FILE").or_else(|| std::env::var_os("HERDR_CONFIG_PATH"))
    {
...
/// Where Herdr keeps `config.toml`.
///
/// Herdr follows each platform's own convention rather than XDG everywhere:
/// `~/.config/herdr` on macOS and Linux, `%APPDATA%\herdr` on Windows. Writing
/// to the wrong one silently produces a config the server never reads, so this
/// is resolved rather than assumed.
fn herdr_config_dir() -> Result<PathBuf> {
    if let Some(config) = std::env::var_os("XDG_CONFIG_HOME") {
        return Ok(PathBuf::from(config).join("herdr"));
    }
    #[cfg(windows)]
    {
        if let Some(appdata) = crate::platform::roaming_data_dir() {
            return Ok(appdata.join("herdr"));
        }
    }
    Ok(crate::platform::home_dir()?.join(".config").join("herdr"))
}
...
fn installed_path() -> Result<Option<PathBuf>> {
    Ok(std::env::var_os("HERDR_PLUGIN_STATE_DIR")
        .map(PathBuf::from)
        .map(|directory| directory.join(CONFIG_INSTALLED_FILE)))
}

fn exact_restore_is_current(current: &str) -> Result<bool> {
    let Some(installed) = installed_path()? else {
        return Ok(false);
    };
    if installed.exists() {
        return Ok(
            fs::read(&installed).context("read Herdr installed-config snapshot")?
                == current.as_bytes(),
        );
    }
    Ok(backup_path()?.is_some_and(|backup| !backup.exists()))
}

fn record_installed_config(installed: &str, exact_restore: bool) -> Result<()> {
    let Some(path) = installed_path()? else {
        return Ok(());
    };
    if exact_restore {
        fs::write(&path, installed).context("write Herdr installed-config snapshot")
    } else {
        remove_if_exists(&path).context("invalidate Herdr installed-config snapshot")
    }
}
    ...
    for (fields, brand) in variants {
        for layout in [SidebarLayout::Packed, SidebarLayout::Stacked] {
            for gap in [SidebarRowGap::FLUSH, SidebarRowGap::SEPARATED] {
                if add_quota_row_with_dashboard(
                    original,
...
fn add_quota_row_with_dashboard(
    input: &str,
    agents: &[Harness],
    layout: SidebarLayout,
    row_gap: SidebarRowGap,
    ...
    *rows = updated_rows;
    let rows = rows.clone();
    if brand.is_on() {
        add_provider_rows(table, &rows, agents, dashboard)?;
    } else {
...
pub fn remove_quota_row_for(input: &str, agents: &[Harness], full: bool) -> Result<String> {
    remove_quota_row_for_with_dashboard(input, agents, full, &DashboardPreferences::default())
}

fn remove_quota_row_for_with_dashboard(
    input: &str,
    agents: &[Harness],
    full: bool,
    dashboard: &DashboardPreferences,
...
fn remove_plugin_keybindings(document: &mut DocumentMut) {
    ...
    for command in commands.iter() {
        if !matches!(
            ...
            Some(REFRESH_ACTION | SETTINGS_ACTION | DASHBOARD_ACTION)
        ) {
...
fn add_provider_rows(
    ...
    for (harness, provider, brand, dim) in selected_styles(agents) {
        ...
        let brand = dashboard_provider(harness)
            .map(|provider| dashboard.get(provider).color.as_str())
            .or(brand);
...
fn remove_managed_provider_rows(table: &mut Table, agents: &[Harness]) {
    ...
    for (_, provider, _, _) in selected_styles(agents) {
        let is_managed = rows_by_agent
            .get(provider)
            .and_then(Item::as_value)
            .is_some_and(has_provider_style_marker);
...
fn dashboard_provider(harness: Harness) -> Option<DashboardProvider> {
    Some(match harness {
        Harness::Claude => DashboardProvider::Claude,
        Harness::Codex => DashboardProvider::Codex,
        Harness::Grok => DashboardProvider::Grok,
        Harness::Agy => DashboardProvider::Agy,
        Harness::OpenCode => DashboardProvider::OpenCodeGo,
        Harness::Omp => DashboardProvider::Omp,
        Harness::Hermes => DashboardProvider::Hermes,
        Harness::Pi => DashboardProvider::Codex,
    })
}
...
fn is_official_state_row(row: &Array) -> bool {
    row.iter().any(|item| {
        item.as_str() == Some("state_icon")
            || configured_token_name(item).is_some_and(|token| token.starts_with("$state_"))
    })
}
...
fn append_quota_rows(rows: &mut Array, layout: SidebarLayout) {
    // Herdr drops empty tokens and empty rows. Packed keeps diagnostics on one
    // line and both visible quota slots on the next; stacked gives every field
    // its own line without changing which metadata names are published.
    for row in rows.iter_mut() {
        let Some(items) = row.as_array_mut() else {
            continue;
        };
        let has_state_icon = is_official_state_row(items);
        let mut cleaned = Array::new();
    ...
    *rows = compacted_rows;
    ...
    let official_index = rows
        .iter()
        .position(|row| row.as_array().is_some_and(is_official_state_row));
...
fn append_packed_quota_rows(rows: &mut Array) {
    rows.push(Value::Array(Array::from_iter([
        styled_token("$quota_context", None, Some(false), Some(false)),
...
fn append_window_style_tokens(row: &mut Array, base: &str) {
    // One compact token per window (`5h 0% reset 1h18m`). Herdr joins sibling
...
"#;
    ...
    #[test]
    fn packed_layout_keeps_diagnostics_together_and_quota_on_its_own_row() {
        let updated =
        ...
        assert!(context_row.iter().all(|item| {
            configured_token_name(item).is_none_or(|token| {
                !token.starts_with("$quota_5h") && !token.starts_with("$quota_week")
            })
        }));
    ...
    #[test]
    fn packed_layout_recognizes_current_herdr_state_tokens() {
        let original = r##"[ui.sidebar.agents]
rows = [["workspace"], [{ token = "$state_working", fg = "#eaa57a" }, { token = "$state_idle", fg = "#aebeb9" }], [{ token = "terminal_title_stripped", fg = "#ffffff" }]]
"##;
        let updated = add_quota_row(original).unwrap();
        assert_eq!(add_quota_row(&updated).unwrap(), updated);
        ...
        let identity = rows
            .iter()
            .find(|row| row_contains_token(row, "$state_working"))
            .unwrap();
        assert!(row_contains_token(identity, "$quota_provider_model"));
    }
    ...
    #[test]
    fn puts_context_cache_rate_and_remaining_ttl_on_one_row() {
        let updated =
            add_quota_row("[ui.sidebar.agents]\nrows = [[\"state_icon\", \"agent\"]]\n").unwrap();
        let document = updated.parse::<DocumentMut>().unwrap();
        let rows = document["ui"]["sidebar"]["agents"]["rows"]
    ...
    fn stacked_layout_puts_each_quota_field_on_its_own_row() {
        ...
        assert!(!updated.contains("$quota_week_inline"));
        assert!(!rows.iter().any(|row| {
...
mod field_tests {
    ...
    #[test]
    fn dashboard_defaults_color_supported_sidebar_harnesses() {
        let branded = applied(FieldSet::all(), BrandColors::On);
        let omp = branded.split("omp =").nth(1).expect("omp row");
        assert!(omp.lines().next().unwrap_or_default().contains("#AA8EFF"));
    ...
    #[test]
    fn custom_provider_color_never_replaces_quota_severity_colors() {
        let mut dashboard = DashboardPreferences::default();
        dashboard.get_mut(DashboardProvider::Codex).color = "#123456".to_string();
        dashboard.get_mut(DashboardProvider::OpenCodeGo).color = "#234567".to_string();
        let branded = add_quota_row_with_dashboard(
            "",
            &[Harness::Codex, Harness::OpenCode, Harness::Pi],
            SidebarLayout::Packed,
            SidebarRowGap::default(),
            FieldSet::all(),
            BrandColors::On,
        ...
        .unwrap();
        let codex = branded.split("codex =").nth(1).expect("codex row");
        assert!(codex.lines().next().unwrap_or_default().contains("#123456"));
        let pi = branded.split("pi =").nth(1).expect("pi row");
        assert!(pi.lines().next().unwrap_or_default().contains("#123456"));
        let opencode = branded.split("opencode =").nth(1).expect("opencode row");
        assert!(opencode
            .lines()
            .next()
            .unwrap_or_default()
            .contains("#234567"));
        assert!(branded.contains(QUOTA_SAFE_COLOR));
        assert!(branded.contains(QUOTA_WARNING_COLOR));
        assert!(branded.contains(QUOTA_DANGER_COLOR));
    }
