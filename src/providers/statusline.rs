...
pub fn enrich_cache_session(
    ...
    let session_id = crate::model::private_identifier("session", session_id);
    ...
    let matching_previous = previous_cache
        .filter(|previous| previous.session_id.as_deref() == Some(session_id.as_str()));
