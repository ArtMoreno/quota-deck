...
mod tests {
    ...
    fn the_cli_is_called_for_one_provider_and_its_report_is_parsed() {
        ...
        let report = serde_json::to_string(&anthropic_report()).unwrap();
        // `Command` needs a file the platform's loader can execute: a `#!`
        // script on Unix, a batch file on Windows, which `Command` routes
        // through `cmd.exe`.
        let stub = if cfg!(windows) {
            dir.path().join("omp-stub.cmd")
        } else {
            dir.path().join("omp-stub")
        };
        let body = if cfg!(windows) {
            format!(
                "@echo off\r\n>\"{}\" echo %*\r\necho {}\r\n",
                arguments.display(),
                // `cmd.exe` expands `%...%` as it reads each batch line; `%%`
                // is the escape for a literal percent.
                report.replace('%', "%%")
            )
        } else {
            format!(
