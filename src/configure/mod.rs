...
use crate::dashboard_prefs::DashboardPreferences;
...
pub fn run(
    ...
    if apply || uninstall {
        std::env::var_os("HERDR_PLUGIN_STATE_DIR").context(
            "configuration writes must run through Herdr so every collector uses the same cache; invoke herdr-agent-quota-win.configure or herdr-agent-quota-win.uninstall",
        )?;
    }
    if agents.is_empty() {
    ...
    if uninstall {
        ...
        let glyphs = resolved_brand_glyphs(options.brand_glyphs, Some(&cache));
        cache.set_brand_glyphs(glyphs)?;
        prefs::write(prefs::BRAND_GLYPHS, glyphs.as_str())?;
        ...
        herdr::apply(agents, layout, gap, fields, brand, &dashboard)?;
        ...
        let dashboard = cache
            .as_ref()
            .map(DashboardPreferences::load_read_only)
            .transpose()?
            .unwrap_or_default();
        let glyphs = resolved_brand_glyphs(options.brand_glyphs, cache.as_ref());
        ...
        herdr::check(agents, layout, gap, fields, brand, &dashboard)?;
        println!("Quota percentages show {} quota.", percent.suffix());
        println!("Brand glyphs: {}.", glyphs.as_str());
...
pub(crate) fn resolved_brand_glyphs(
    explicit: Option<GlyphSet>,
    cache: Option<&CacheStore>,
) -> GlyphSet {
    explicit
        .or_else(|| {
            std::env::var(GlyphSet::ENV)
                .ok()
                .as_deref()
                .and_then(GlyphSet::parse)
        })
        .or_else(|| prefs::read(prefs::BRAND_GLYPHS).and_then(|value| GlyphSet::parse(&value)))
        .or_else(|| cache.and_then(CacheStore::brand_glyphs))
        .unwrap_or_default()
}
